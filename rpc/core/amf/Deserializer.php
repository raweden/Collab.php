...
class AmfDeserializer{
	...
	protected function readHeaders(){		
		...
		if(!($topByte == 0 || $topByte == 3)){
			throw new RemotingException("Malformed Amf Packet, connection may have dropped");
		}
	...
	public function readData($type){
		switch ($type){
			...
			case 0X0D: // mainly internal AS objects
				return null;
            			//ignore recordset (reserverd, not supported)
			case 0X0F: // XML
				return $this->readXml();
			case 0x10: // Custom Class
				return $this->readCustomClass();
			default: // unknown case
				throw new RemotingException("Found unhandled type with code: $type");
				exit();
				break;
		}
		return $data;
	...
	*/
	public function readAmf3Data(){
    	...
    	switch ($type){
			...
			default:
				throw new RemotingException("undefined Amf3 type encountered: " . $type);
		}
	}
	...
	protected function readAmf3Date(){
		...
		if (($firstInt & 0x01) == 0){
			...
			if ($firstInt >= count($this->storedObjects)){
				throw new RemotingException('Undefined date reference: ' . $firstInt);
				return false;
			}
	...
	protected function readAmf3String(){
		...
		if (($strref & 0x01) == 0){
			...
			if ($strref >= count($this->storedStrings)){
				throw new RemotingException('Undefined string reference: ' . $strref, E_USER_ERROR);
				return false;
			}
