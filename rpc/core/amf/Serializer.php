...
class AmfSerializer {
	 ...
	 */
	protected function writeDate(Date $d) {
		$this->writeByte(0x0B); 
	...
	protected function writeData($d) {
		...
		throw new RemotingException("couldn't write data ");
	}
	...
	protected function writeAmf3Xml(XML $d) {
		$d = preg_replace('/\>(\n|\r|\r\n| |\t)*\</', '><', trim($d->data));
		$this->writeByte(0x0B);
		$this->writeAmf3String($d);
	}

	protected function writeAmf3XmlDocument(XMLDocument $d) {
		$d = preg_replace('/\>(\n|\r|\r\n| |\t)*\</', '><', trim($d->data));
		$this->writeByte(0x07);
		$this->writeAmf3String($d);
	}

	protected function writeAmf3Date(Date $d) {
		$this->writeByte(0x08);
		$this->writeAmf3Int(1);
		$this->writeDouble($d->timeStamp);
	}
