...
class ServiceBrowser implements IDeserializer, IDeserializedRequestHandler, IExceptionHandler, ISerializer {
	...
	public function serialize($data) {
		...
		if($this->serviceName){
			...
			foreach ($availablePublicMethods as $methodDescriptor){
				...
				if($this->isHiddenMethod($availableMethodName)){
					continue;	
				}
		...
		if($this->methodName){
			...
			if (count($parameterDescriptors) > 0) {
				...
				foreach ($parameterDescriptors as $parameterDescriptor) {
					...
					// if the parameter name is password, then set input type to password.
					if($availableParameterName == "password"){
		...
		if($this->showResult){
			...
			$message .= "<div class=\"infobar\" style=\"height: 20px;\"><span style=\"float: right;height: 20px;line-height: 20px;\" >" . $this->resultType($data) . "</span><span style=\"float: left;height: 20px;line-height: 20px;\" >Result</span></div>";
			//
			$message .= "<div class=\"wikistyle content\"><pre style=\"max-height:480px;\"><code>";
			$message .= $this->printResult($data);
		}
		$message .= "<code></pre><div></body></html>";		
		return $message; 
	}
	...
	private function resultType($data){
		if(is_string($data)){
			return "String";
	 ...
	 * @author Raweden.
	 */
	private function isHiddenMethod($methodName){
		return substr($methodName,0,2) == "__" || substr($methodName,0,1) == "_" || $methodName == "beforeFilter";
	}
