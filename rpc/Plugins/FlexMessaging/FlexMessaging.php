...
class FlexMessaging{
    ...
    public function  __construct(array $config = null) {
        FilterManager::getInstance()->addFilter(Amfphp_Core_Amf_Handler::FILTER_AMF_REQUEST_MESSAGE_HANDLER, $this, "filterAmfRequestMessageHandler");
        FilterManager::getInstance()->addFilter(Amfphp_Core_Amf_Handler::FILTER_AMF_EXCEPTION_HANDLER, $this, "filterAmfExceptionHandler");
    ...
    public function handleRequestMessage(Amfphp_Core_Amf_Message $requestMessage, Amfphp_Core_Common_ServiceRouter $serviceRouter){
        ...
        if($messageType == self::FLEX_TYPE_COMMAND_MESSAGE){
            ...
            $acknowledge = new FlexMessaging_AcknowledgeMessage($requestMessage->data[0]->$messageIdField);
        ...
        if($messageType == self::FLEX_TYPE_REMOTING_MESSAGE){
            ...
            $acknowledge = new FlexMessaging_AcknowledgeMessage($remoting->$messageIdField);
    ...
    public function generateErrorResponse(Exception $exception){
        $error = new FlexMessaging_ErrorMessage($this->lastFlexMessageId);
